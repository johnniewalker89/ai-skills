...
- Owns: task mode, local context workspace, `environment.md` lifecycle and local runtime resolution, task snapshots, optional agent logs, task navigation, planning, approval acquisition/retention and stop behavior, decision-grade checkpoints, validation framing, and final self-review.
...
6. **External-action approval-source gate.** Before any selected owner marks an external mutation, communication, control action, privileged access, sensitive read, or destructive action approval-gated—including message send/edit/delete, document publication, queue/broker mutation, CI/deploy, or service control—require a fresh visible user message in the current chat after presenting the exact contour, action, target set, and impact. For communication or documentation, the checkpoint must also show the complete final payload, including title/body/text and attachment names when applicable; drafting stays approval-free, but any target or payload change after approval requires a new checkpoint. One fresh reply may approve an explicitly enumerated batch of unchanged exact payloads, never future or unspecified communication. Run any owner-defined bounded read or prepare call before asking; never request preliminary or general approval merely to perform that approval-free preparation, and make the first approval prompt the post-prepare exact checkpoint. Local full-access or never-ask mode, a `functions.exec`/programmatic tool call, client auto-approval or prompt configuration, an agent-supplied flag such as `approved=true`, an earlier general instruction, or another target's approval/session is not user consent. A short reply such as `да` or `continue` counts only as the direct answer to the unchanged exact checkpoint. Reuse an owner-returned server-bound session only inside its declared scope, TTL, and action cap; without such a session each exact action is one-shot. Restart, drift, expiry, or scope change requires fresh approval.
7. **Database access gate.** Use `db-access` for direct MCP metadata, catalog, DDL, `EXPLAIN`, query-log checks, smoke queries, and privileged introspection. A non-`db-access` route is valid only when a separately installed access owner defines a typed read-only tool, its complete SQL dependency chain, and exact current approval for the call. A missing or denied contour never authorizes raw SSH database commands.
8. **Privileged/sandbox action gate.** Before any database state-changing action, sandbox CTAS/materialization, cleanup/drop, rebuild, recalculation, or privileged database-tool use, the current task must have explicit approval for the exact database contour, action type, target set, and cleanup/rollback expectation. General approval to continue, edit repo files, prove an idea, or use a sandbox is not enough for privileged database actions. A default MCP outage or timeout is a blocker/escalation question, not permission to switch contours.
9. **Project gate.** For `project` mode, risky/expensive work, destructive/hard-to-reverse work, unclear business rules, or cross-system work, read `references/safety_and_stop_points.md` before implementation or approval requests.
10. **New production-like data artifact gate.** For new marts, models, tables, reports, data contracts, or test/sandbox equivalents of production-like artifacts, choose `project` mode and read both `references/safety_and_stop_points.md` and `references/validation_and_review.md` before creating solution artifacts or asking for approval.
11. **Artifact destination gate.** Code artifacts belong in the target repo/workspace after approval. The context workspace is for task notes, agent logs, decisions, and evidence unless the user explicitly asks for a context-only draft/package. Read `references/safety_and_stop_points.md` before creating solution artifacts.
12. **Cleanup/deletion usage-proof gate.** For repo cleanup, deletion, deprecation, DAG/report/job removal, live cleanup/drop lists, or hard-to-reverse cleanup recommendations, read `references/safety_and_stop_points.md` before editing or recommending removal. Prove ownership and usage with read-only evidence first; keep repo cleanup separate from live state changes.
13. **Validation/proof gate.** Before final delivery for `focused`, `project`, `investigation`, `review`, SQL/dbt/data-pipeline work, or production-like artifacts, read `references/validation_and_review.md`. Final status must match the strongest completed proof level.
...
- Did ordinary delivery keep zero writes under the configured skill-review/evaluation tree, leaving task-row discovery and reconciliation to a separate evaluator while updating only navigation, snapshots, and enabled logs?
...
- Before every approval-gated external action, did approval-free preparation happen before the first prompt, and did one fresh visible current-chat user message bind the resulting exact checkpoint—including the complete final payload for communication/documentation—with client/full-access settings and agent-supplied flags treated only as transport rather than consent?
- Did I reuse approval only through the owning server's still-valid bound session and require a fresh one-shot approval when no such session covered the action?
