...
- Before every approval-gated external action, did approval-free preparation happen before the first prompt, and did one fresh visible current-chat user message bind the resulting exact checkpoint, with client/full-access settings and agent-supplied flags treated only as transport rather than consent?
