...
4. **Agent-log gate.** All agent logs are opt-in. One request such as “enable agent logs” enables main and subagent logs in the requested task/initiative scope; retain that state across continuations. Read `references/agent_logging.md` when enabling or using logs. A task id or delegation alone does not enable them. Workflow owns the shared flag, storage and navigation; orchestration consumes it and owns per-attempt logging. With logging off, create no audit logs/index; required work results and proof still apply.
...
- `references/agent_logging.md`: when enabling, maintaining or navigating main/subagent logs and planning history.
