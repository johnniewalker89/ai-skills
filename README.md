...
- `subagent-orchestration` — планирование субагентов, бюджеты с запасом, согласование запуска, контроль прогресса и сохранение результатов.
...
Перед планированием, согласованием или управлением субагентами `agent-workflow-core` обязательно подключает `subagent-orchestration`, в том числе если такая необходимость возникла по ходу задачи. Отдельный навигатор для этого не нужен. Можно вызвать `subagent-orchestration` напрямую; он использует workflow как companion. Обычная работа без субагентов не загружает этот skill.

## Что нужно заранее

- Установить эти skills в agent runtime.
- При выборочной установке ставить `agent-workflow-core` и `subagent-orchestration` вместе из одной revision: это условная зависимость при работе с субагентами, а не автоматическое разрешение их запускать.
- Настроить `profi-mcp`.
...
2. Завести в ней минимум:
   - `README.md` — краткую карту владельцев информации;
   - `context.md`
   - `environment.md`
   - `tasks/`
...
Агент выбирает минимальную полную цепочку workflow/quality skills и восстанавливает нужный контекст существующей задачи. Обычные разрешённые read-only действия не требуют дополнительного согласования; checkpoints определяет владелец операции с учётом уже предоставленных разрешений.

## Логирование

Краткий task context ведётся для уже заведённой context-backed задачи или по указанным выше условиям его включения. Если нужен отдельный подробный ход работы, добавь в запрос:
...
Skills не хранят личные пути, credentials и task snapshots внутри себя. Они являются переносимым ядром, а локальные детали живут в выбранной пользователем рабочей области контекста. Доступ к приватной репе автора не нужен.
