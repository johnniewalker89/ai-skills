...
6. **Control gate.** Verify child identity and monitor usage plus semantic progress. Inspect the first write failure or missed report checkpoint promptly. A checkpoint requests assessment, not automatic FINISH. Use work → finishing → save/readback → delivery; begin budget-driven finishing at the working upper bound. Earlier completion needs a solved task or concrete blocker/failed path, not merely a missing draft or the lower forecast being reached. Once finishing is instructed or final saving is observed, latch protected completion: no token/time/progress deadline may interrupt saving, required readback or delivery, even past the original ceiling. This authorizes no new research; record any overrun. Apply the protection after caller-specific controls, immediately before dispatch. Explicit user stops and access boundaries retain priority.
...
8. **Calibration gate.** Assess working-forecast accuracy separately from completion cost and total ceiling compliance; final completion inside headroom is its intended use. Lower headroom only after studying comparable evidence and obtaining the user's decision; one success does not establish calibration. Apply supplied local planning policy without making it a public-package dependency.
...
- Completion state retained through save/readback/delivery, including before a finish signal; no budget interrupt, hidden cap increase or new research in that phase?
