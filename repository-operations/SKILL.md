...
5. **Technical-grant gate.** Ordinary technical work may use one short task-scoped approval only when bound to provider/host, immutable account/repository ids, workspace realpath + Git-dir identity, task/project, exact branch, allowed actions + paths, expected local/remote SHA, short TTL, process nonce, and finite use cap. Enforce compare-and-swap before every use; drift, restart, replay, nonce/TTL/use exhaustion, or CAS mismatch invalidates the grant. It never covers communication, protected branches, force/destructive Git, deploy/publish/release, credentials, secrets, repository control-plane configuration, or administration. A direct unprotected default-branch grant is valid only for the exact personal repository explicitly enabled by server policy; no other repository in the chat inherits it.
...
- Did I keep communication, protected branches, deploy/release, secrets, credentials, and repository administration outside the technical grant, and allow a default-branch push only for an explicitly configured personal repository policy?
